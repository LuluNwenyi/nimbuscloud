...
6. And that’s it! You’re done.
  ...
  2. After uploading your file to NimbusCloud, you can also use the [`ListFile` endpoint](/api-reference#tag/nimbusstore/GET/cloud/files/%7Bfile_id%7D) to retrieve objects from the NimbusStore, and the [`DeleteFile` endpoint](api-reference#tag/nimbusstore/DELETE/cloud/files/%7Bfile_id%7D) to delete objects you no longer need from NimbusStore.
  :::
